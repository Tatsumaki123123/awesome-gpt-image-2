...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchJsonWithFallback = async (primaryUrl, fallbackUrl) => {
      const primary = await fetch(primaryUrl).catch(() => null);
    ...
    Promise.all([
      fetchJsonWithFallback('/api/v1/site-data', '/cases.json'),
      fetchJsonWithFallback('/api/v1/style-library', '/style-library.json')
    ])
